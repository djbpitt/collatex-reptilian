...
class HypergraphTest extends AnyFunSuite:
  ...
  test("overlay with two hyperedges"):
    ...
    val expectedHyperedgeLabels = Set("edge1", "edge2")
    assert(result.verticesIterator.toSet == expectedVertices)
    assert(result.hyperedges.map(_.label) == expectedHyperedgeLabels)
  ...
  test("connect; basic connect plus lonely vertices"):
    ...
    assert(result("edge1").getOrElse(fail("Should not happen")).verticesIterator.toSet == expectedMembers)
  ...
  test("connect; basic connect plus lonely hyperedges"):
    ...
    assert(result("edge1").getOrElse(fail("Should not happen")).verticesIterator.toSet == Set(1))
    assert(result("edge2").getOrElse(fail("Should not happen")).verticesIterator.toSet == Set.empty)
    assert(result.edges(1) == Set("edge1"))
    assert(result.hyperedges.map(_.label) == Set("edge1", "edge2"))
  ...
  test("connect; duplicated hyperedge; different vertices"):
    ...
    assert(result.hyperedges.map(_.label) == Set("edge1"))
    assert(result.verticesIterator.toSet == Set(1, 2))
    assert(result("edge1").getOrElse(fail("Should not happen")).verticesIterator.toSet == Set(1, 2))
    assert(result.edges(1) == Set("edge1"))
    assert(result.edges(2) == Set("edge1"))
  ...
  test("connect; duplicated vertex; different hyperedges"):
    ...
    assert(result("edge1").getOrElse(fail("Should not happen")).verticesIterator.toSet == Set(1))
    assert(result("edge2").getOrElse(fail("Should not happen")).verticesIterator.toSet == Set(1))
