...
val loggerSanity = LoggerFactory
...
def sanityLogging(root: AlignmentRibbon, gTa: Vector[TokenEnum]): Unit = {
  if effectiveLevel == Level.DEBUG then
    ...
    if duplicateTokens.isEmpty && missingTokens.isEmpty then loggerSanity.debug("No duplicate or missing tokens")
    else
      if duplicateTokens.nonEmpty then loggerSanity.debug(s"Duplicate tokens: $duplicateTokens")
      if missingTokens.nonEmpty then loggerSanity.debug(s"Missing tokens: $missingTokens")
}
